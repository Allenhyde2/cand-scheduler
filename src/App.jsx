  ...
  const fetchProductsWithArgs = async (currentToken, currentCommunityId, currentSellerId, currentMode) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        method: 'GET', headers: getAuthHeaders(activeToken, activeCommunityId)
      });
        ...
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          ...
          <div className="flex border-b border-gray-200">
            <button 
              ...
              className={`flex-1 py-3 text-sm font-bold transition ${loginMode === 'seller' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
              판매자용 (Seller)
            </button>
            <button 
              onClick={() => setLoginMode('admin')}
              className={`flex-1 py-3 text-sm font-bold transition ${loginMode === 'admin' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
              어드민용 (Admin)
            </button>
          </div>
          ...
          <form onSubmit={handleOAuthLogin} className="p-8 space-y-6">
            ...
            {loginMode === 'seller' ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">조회할 판매자 ID <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                    value={sellerId}
            ...
            <button 
              ...
              className="w-full text-white font-bold py-3.5 rounded-xl hover:opacity-90 transition-opacity flex justify-center items-center shadow-md disabled:opacity-50"
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-0 hidden'} flex-shrink-0 bg-white border-r border-gray-200 transition-all duration-300 flex flex-col`}>
        ...
        <nav className="flex-1 overflow-y-auto py-4">
          ...
          <ul className="space-y-1 px-3 mb-6">
            <li>
              <button onClick={() => setActiveTab('productList')} className={`w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium ${activeTab === 'productList' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50'}`}>
                판매 상품 목록
              </button>
            </li>
            <li>
              <button onClick={() => setActiveTab('schedule')} className={`w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium ${activeTab === 'schedule' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50'}`}>
                상태 예약 변경
              </button>
            </li>
          </ul>
        ...
        <div className="p-4 border-t border-gray-100">
          <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-sm text-gray-500 hover:text-red-500 font-bold">
            로그아웃
          </button>
        </div>
      </aside>
